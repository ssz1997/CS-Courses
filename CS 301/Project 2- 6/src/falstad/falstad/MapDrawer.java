...
public class MapDrawer extends DefaultViewer {
	...
	public MapDrawer(int width, int height, int map_unit, int step_size, Cells seencells, int map_scale){
		...
		controller = MazeApplication.controller;
	...
	public void redraw(MazePanel panel) {
		...
		if (null != controller) {
			if (controller.isInMapMode()) {
				draw_map(controller.getPanel()) ;
				draw_currentlocation(controller.getPanel(), controller.getViewdxdy()[0], controller.getViewdxdy()[1]) ;
			}
		}
	}
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				if ((seencells.hasWall(x,y, CardinalDirection.North) || controller.isInShowMazeMode()) && theCondition)
					panel.getBufferGraphics().drawLine(nx1, ny1, nx2, ny1);
				...
				if ((seencells.hasWall(x,y, CardinalDirection.West) || controller.isInShowMazeMode()) && theCondition)
					panel.getBufferGraphics().drawLine(nx1, ny1, nx1, ny2);
			}
	...
	public void draw_currentlocation(MazePanel panel, int view_dx, int view_dy) {
		...
		panel.getBufferGraphics().fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
