...
public class BasicRobot implements Robot{
	...
	public void rotate(Turn turn) {
		if (batteryLevel >= 3) {
			if (turn == Turn.LEFT) {
				mazeController.operateGameInPlayingState(UserInput.Left);
				batteryLevel -= 3;
				mazeController.setEnergyConsumption(3000-batteryLevel);
			}
			else if (turn == Turn.RIGHT) {
				mazeController.operateGameInPlayingState(UserInput.Right);
				batteryLevel -= 3;
				mazeController.setEnergyConsumption(3000-batteryLevel);
			}
			else {
		...
		if (manual) {
			for (int steps = 0; steps < distance; steps ++) {
				if (!hasStopped() && batteryLevel >= 5) {
					mazeController.operateGameInPlayingState(UserInput.Up);
					batteryLevel -= 5;
					mazeController.setEnergyConsumption(3000-batteryLevel);
					odometer += 1;
	...
	public boolean canSeeExit(Direction direction) throws UnsupportedOperationException {
		...
		if (currentPosition[0] != cells.getExit()[0] && currentPosition[1] != cells.getExit()[1]) {
			...
			if (currentPosition[0] == cells.getExit()[0] && currentPosition[1] == cells.getExit()[1]) {
			    if (cells.hasNoWall(currentPosition[0], currentPosition[1], cd) && !mazeController.getMazeConfiguration().isValidPosition(currentPosition[0] + cd.getDirection()[0], currentPosition[1] + cd.getDirection()[1])){
			    	    return true;
			    }
			    return false;
			}
