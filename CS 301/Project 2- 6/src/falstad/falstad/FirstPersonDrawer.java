...
public class FirstPersonDrawer extends DefaultViewer {
	...
	public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells seencells, BSPNode bsp_root) {
		...
		this.controller = MazeApplication.controller;
	}
	...
	private void drawBackground(MazePanel panel) {
		panel.setColorBlack();
		panel.getBufferGraphics().fillRect(0, 0, view_width, view_height/2);
		panel.setColorDarkGray();
		panel.getBufferGraphics().fillRect(0, view_height/2, view_width, view_height/2);
	}
	/**
	...
	private void drawAllSegmentsOfASector(BSPLeaf n) {
		...
		for (int i = 0; i != sl.size(); i++) {
			...
			drawSegment(seg, controller.getPanel());
			// debug
			if (deepdebug) {
				dbg("                               ".substring(0, nesting) +
						" traverse_ssector(" + i +") "+
	...
	private void drawSegment(Seg seg, MazePanel panel) {
		...
		panel.getBufferGraphics().setColor(panel.objectToColor(seg.getColor()));
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			panel.getBufferGraphics().fillPolygon(xps, yps, 4);
