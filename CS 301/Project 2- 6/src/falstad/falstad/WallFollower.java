...
public class WallFollower implements RobotDriver {
	...
	public boolean drive2Exit() throws Exception {
		...
		int distanceToLeft = robot.distanceToObstacle(Direction.LEFT);
		int distanceToFront;
		if(distanceToLeft != 0) {
		...
		while(!robot.isAtExit()) {
			...
			if(robot.getBatteryLevel()>=1) {
			    distanceToLeft = robot.distanceToObstacle(Direction.LEFT);
			}
			else throw new Exception();
			
			if(robot.getBatteryLevel()>=1) {
			    distanceToFront = robot.distanceToObstacle(Direction.FORWARD);
			}
			else throw new Exception();
