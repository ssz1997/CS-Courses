...
public class FirstPersonDrawer extends DefaultViewer {
	...
	@Override
	public void redraw(MazeController mazeController, MazePanel panel) {
        ...
        System.out.println("mazePanel in redraw in first person drawer: " + panel);
		Log.v("firstpersondrawer","redraw called");
		if (controller.getState() != Constants.StateGUI.STATE_PLAY)
			return ;

		this.rset = controller.getRangeSet() ;
		this.view_dx = controller.getViewdxdy()[0] ;
		this.view_dy = controller.getViewdxdy()[1] ;
		this.angle = controller.getAngle() ;
		this.mazePanel = panel;
		// calculate view
		System.out.println("LOOK AT HERE!!!");
		System.out.println(controller.getCurrentPosition()[0]);
		System.out.println(controller.getCurrentPosition()[1]);
		viewx = (controller.getCurrentPosition()[0]*map_unit+map_unit/2) + viewd_unscale(view_dx*(step_size*controller.getWalkStep()-Constants.VIEW_OFFSET));
		viewy = (controller.getCurrentPosition()[1]*map_unit+map_unit/2) + viewd_unscale(view_dy*(step_size*controller.getWalkStep()-Constants.VIEW_OFFSET));
		// update graphics
		...
		Log.v("firstPersonDrawer Counter",String.valueOf(counter));
		if (counter % 4 == 1  ) {
	...
	private void drawSegment(Seg seg) {
		...
		while (x1i <= x2) {
			...
			mazePanel.addToDraw(xps);
			mazePanel.addToDraw(yps);
			System.out.println("mazePanel to draw" + mazePanel);
