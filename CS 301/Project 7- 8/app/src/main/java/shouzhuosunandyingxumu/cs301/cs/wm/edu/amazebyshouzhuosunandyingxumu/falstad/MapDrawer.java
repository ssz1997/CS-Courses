...
public class MapDrawer extends DefaultViewer {
	 ...
	 * @param map_scale
	 */
	public MapDrawer(MazeController mazeController, int width, int height, int map_unit, int step_size, Cells seencells, int map_scale){
	...
	public void redraw(MazeController mazeController, MazePanel mazePanel) {
		//dbg("redraw") ;
		//px = controller.getCurrentPosition()[0];
		//diew_dx = controller.getViewdxdy()[0];
		//rset = controller.getRangeSet();
		this.controller = mazeController;
		Log.v("map counter",String.valueOf(counter));
		if (controller.getState() != Constants.StateGUI.STATE_PLAY)
			return ;
		if (null != controller) {
			if (controller.isInMapMode()) {
				if (counter % 4 == 1) {
					Log.v("map counter draw", String.valueOf(counter));
					draw_map(mazePanel);
					draw_currentlocation(mazePanel, controller.getViewdxdy()[0], controller.getViewdxdy()[1]);
	 ...
	 */
	public void draw_map(MazePanel panel) {
		...
		int vx = controller.getCurrentPosition()[0]*map_unit+map_unit/2;
		vx += viewd_unscale(controller.getViewdxdy()[0]*(step_size*controller.getWalkStep()));
		int vy = controller.getCurrentPosition()[1]*map_unit+map_unit/2;
		vy += viewd_unscale(controller.getViewdxdy()[1]*(step_size*controller.getWalkStep()));
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				if ((seencells.hasWall(x,y, CardinalDirection.North) || controller.isInShowMazeMode()) && theCondition) {
                    int[] line = {nx1, ny1, nx2, ny1};
                    panel.addLineNorth(line);
				...
				if ((seencells.hasWall(x,y, CardinalDirection.West) || controller.isInShowMazeMode()) && theCondition) {
                    int[] line = {nx1, ny1, nx1, ny2};
                    panel.addLineWest(line);

                }
			}
		
		if (controller.isInShowSolutionMode()) {
			draw_solution(panel, offx, offy, controller.getCurrentPosition()[0], controller.getCurrentPosition()[1]) ;
		}
	...
	public void draw_currentlocation(MazePanel panel, int view_dx, int view_dy) {
        ...
        int[] oval = {ctrx-cirsiz/2+15, ctry-cirsiz/2, ctrx+15+cirsiz/2, ctry+cirsiz/2};
        panel.setCurrentPosition(oval);
		...
		// now the drawing
        int[] line1 = {ctrx+15, ctry, aptx+15, apty};
        panel.setCurrentPosition(line1);


		//panel.getBufferGraphics().drawLine(ctrx, ctry, aptx, apty);
        int[] line2 = {aptx+15, apty, aptx2 + ptofx+15, apty2 + ptofy};
        panel.setCurrentPosition(line2);

		//panel.getBufferGraphics().drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
        int[] line3 = {aptx+15, apty, aptx2 - ptofx+15, apty2 - ptofy};
        panel.setCurrentPosition(line3);
		//panel.getBufferGraphics().drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	...
	public void draw_solution(MazePanel panel, int offx, int offy, int px, int py) {
		...
		while (distance > 1) {
			...
			int[] line = {nx1+15, ny1, nx2+15, ny2};
			panel.setLine(line);
			panel.addSolutionLine(line);
			//panel.getBufferGraphics().drawLine(nx1, ny1, nx2, ny2);
