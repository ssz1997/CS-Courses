#This is project 3 of Fall 2017 CSCI 312
#Created by Yining Chen and Shouzhuo Sun
...
def meaning( text, state ) :
        ...
        list = []
        for key in state:
                list.append("<" + str(key) + ", " + str(state[key]) + ">")
        to_print = ", ".join(list)
        to_print = "{" + to_print + "}"
        print (to_print)
        return
...
def main():
	...
	for line in open(sys.argv[1+ct], "r"):
                print (line, end = '')
	meaning("".join(mklines(sys.argv[1+ct])),state)
	return
